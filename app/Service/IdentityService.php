...
class IdentityService implements IdentityServiceInterface
{
    ...
    {
        $request = $this->client->post('https://login.microsoftonline.com/modena.com/oauth2/v2.0/token', [
            'headers' => [
                'Cookie' => 'fpc=Ak8PfC5IyJRMgk3baaPlGBP1lKhyAwAAABqkQdwOAAAA; stsservicecookie=estsfd; x-ms-gateway-slice=estsfd'
            ],
            'multipart' => [
                [
                    'name' => 'client_id',
                    'contents' => '35b5f391-3617-47ec-bb2f-ae3e2251ecec'
                ],
                [
                    'name' => 'scope',
                    'contents' => 'https://graph.microsoft.com/.default'
                ],
                [
                    'name' => 'grant_type',
                    'contents' => 'password'
        ...
        if($code == 400){
            ...
            }else if($errorCode == 50126){
                $message = 'Password do not match';
            }else if($errorCode == 50076){
    ...
    public function loginMicrosoftWithCode($code)
    {
        $request = $this->client->post('https://login.microsoftonline.com/modena.com/oauth2/v2.0/token', [
            ...
            'multipart' => [
                [
                    'name' => 'client_id',
                    'contents' => env('MICROSOFT_CLIENT_ID')
                ],
                [
                    'name' => 'client_secret',
                    'contents' => env('MICROSOFT_SECRET_ID')
                ],
                [
                    'name' => 'scope',
                    'contents' => 'https://graph.microsoft.com/.default'
    ...
    {
        $request = $this->client->get('https://graph.microsoft.com/v1.0/me', [
            'headers' => [
