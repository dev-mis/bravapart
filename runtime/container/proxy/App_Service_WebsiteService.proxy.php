...
class WebsiteService implements WebsiteServiceInterface
{
    ...
    public function __construct()
    {
        ...
        $options = ['handler' => HandlerStack::create(new CoroutineHandler()), 'timeout' => 5, 'http_errors' => false, 'swoole' => ['timeout' => 10, 'socket_buffer_size' => 1024 * 1024 * 2]];
        $this->client = $this->cf->create($options);
    }
    public function create($data)
    {
        $options = [];
        $options['headers'] = ['Authorization' => 'Bearer ' . env('MODENA_API_KEY')];
        $options['json'] = $data;
