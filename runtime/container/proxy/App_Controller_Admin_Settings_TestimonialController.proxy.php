...
class TestimonialController extends AbstractController
{
    ...
    public function store(CreateRequest $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $mediaData = ['content_type' => 'image', 'name' => $file_name, 'file_name' => $file_name, 'path' => 'testimonial/', 'type' => 'image', 'extension' => $file->getExtension(), 'disk' => 'sftp', 'size' => $file->getSize()];
            ...
            $response = ['status' => '200', 'success' => true, 'message' => 'Testimonial created successfully', 'redirect' => url('admin/settings/testimonial')];
            $this->session->set('flash', ['type' => 'success', 'title' => 'Success', 'message' => 'Testimonial created successfully']);
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
    ...
    public function update($id, UpdateRequest $request)
    {
        ...
        try {
            ...
            $response = ['status' => '200', 'success' => true, 'message' => 'Testimonial updated successfully', 'redirect' => url('admin/settings/testimonial')];
            $this->session->set('flash', ['type' => 'success', 'title' => 'Success', 'message' => 'Testimonial updated successfully']);
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
            $message = $e->getMessage();
    ...
    public function delete($id)
    {
        ...
        if (!empty($testimonial)) {
            $data = ['deleted_by' => $this->getAuth()['id'], 'deleted_at' => date('Y-m-d H:i:s')];
            $testimonial = Testimonial::where('id', $id)->update($data);
            $response = ['status' => '200', 'success' => true, 'message' => 'Testimonial deleted successfully', 'redirect' => url('admin/settings/testimonial')];
        } else {
            $response = ['status' => '404', 'success' => false, 'message' => 'Testimonial not found'];
        }
        return $this->sendResponse($response);
    }
}